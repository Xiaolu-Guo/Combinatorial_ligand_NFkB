function data = read_monolix_data_to_matrix_2025_for_newfitted_CpG_pIC(data_proj_num_vec,data_parent_path,cell_num_data)
% input
...
data.exp_mode_filter_nan_neg=cell(2,1);
data.pred_mode_filter_nan_neg=cell(2,1);
...
data.parameters_mode_nan_neg = cell(2,1);
...
for i_proj = 1:length(data_proj_num_vec)
    ...
    [proj_path,proj_name] = subfunc_get_proj_path_2(proj_num,data_parent_path,'XG2025');
    ...
    id_end=cell_num_data;
    id_start=1;
    
    time_points=max(estimates.time)/5+2;
    
    [parameters,para_name] = read_draw_individual_parameters_mode_2025(proj_num,data_parent_path);
    
    for i_sti=1
        data.info_num_cells{i_data} = cell_num_data;
        ...
        for i_cell_each_sti=id_start(i_sti):id_end(i_sti)
            ...
            data.pred_SAEM{i_data}(i_cell,time_celli/5+2)=estimates.indivPred_SAEM(estimates.id==i_cell_each_sti);
            data.pred_mode{i_data}(i_cell,time_celli/5+2)=estimates.indivPred_mode(estimates.id==i_cell_each_sti);
            data.pred_mean{i_data}(i_cell,time_celli/5+2)=estimates.indivPred_mean(estimates.id==i_cell_each_sti);
            data.exp{i_data}(i_cell,time_celli/5+2)=estimates.Y(estimates.id==i_cell_each_sti);
        ...
        index_neg = sum(data.exp_mode_filter_nan{i_data} < 0,2) > 10;
        data.exp_mode_filter_nan_neg{i_data}=data.exp_mode_filter_nan{i_data}(index_neg,:);
        data.pred_mode_filter_nan_neg{i_data}=data.pred_mode_filter_nan{i_data}(index_neg,:);
        data.parameters_mode_nan_neg{i_data} = data.parameters_mode_nan{i_data}(index_neg,:);
...
%             data.(data_field_name{i_field_name}){i_data} = data.(data_field_name{i_field_name}){i_data}(cell_num_data(1),:);
%         end
%         [~, data.order{i_data}]=sort(max(data.exp{i_data},[],2),'descend');
%     end
% end
