function [sim_data_tbl] = NFkB_signaling_para_conc_sim_2025_CpGpolyIC_SR(sim_info,data_info)
...
for i_ligand = 1:length(sim_info.ligand)
    ...
    dose_scale(i_ligand) = get_ligand_dose_scale(dose_ligand{1}); % Convert to uM (from nM)
...
if isfield(sim_info,'ODEmodel_pIC_CpG_compete') && sim_info.ODEmodel_pIC_CpG_compete == 0
    [v0.PARAMS, v0.SPECIES] = nfkbInitialize_pICTLR_deg_increase();
else
    [v0.PARAMS, v0.SPECIES] = nfkbInitialize_CpGPolyIC_SR();
end
...
for i_para_val = 1:size(sim_info.parameter_value_vec,2)% 10%
    ...
    if isempty(output)
        options=options0;
        if isfield(sim_info,'ODEmodel_pIC_CpG_compete') && sim_info.ODEmodel_pIC_CpG_compete == 0
            [~,x,simdata] = nfkbSimulate_pICTLR_deg_increase(stim_input,names, [], {},options);
        else
            [~,x,simdata] = nfkbSimulate_CpGPolyIC_SR(stim_input,names, [], {},options);
        end        
    else
        options.STEADY_STATE = simdata.STEADY_STATE;
        if isfield(sim_info,'ODEmodel_pIC_CpG_compete') && sim_info.ODEmodel_pIC_CpG_compete == 0
            [~,x] = nfkbSimulate_pICTLR_deg_increase(stim_input,names, [], {},options);
        else
            [~,x] = nfkbSimulate_CpGPolyIC_SR(stim_input,names, [], {},options);
        end
    end
...
%% get dose scale coverting ng/mL to uM
function dose_scale_covert_to_uM = get_ligand_dose_scale(dose_ligand)

opts1 = detectImportOptions('MW_Convert_ligand_scale_factor.xlsx','Sheet','Convert_uM');

MW_Convert_ligand_tbl = readtable('MW_Convert_ligand_scale_factor.xlsx',opts1);

categorical_fields = {'Ligand';'convertingTo';'convertFrom';'MW'};

for i_categorical_fields = 1:length(categorical_fields)
    MW_Convert_ligand_tbl.(categorical_fields{i_categorical_fields}) = ...
        categorical(MW_Convert_ligand_tbl.(categorical_fields{i_categorical_fields}) );
end


if ischar(dose_ligand)
    dose_scale = MW_Convert_ligand_tbl.dose_scale(MW_Convert_ligand_tbl.Ligand == dose_ligand);
    dose_scale_covert_to_uM = 1/dose_scale(1);

elseif isnumeric(dose_ligand)
    dose_scale = MW_Convert_ligand_tbl.dose_scale( dose_ligand);
    dose_scale_covert_to_uM = 1/dose_scale(1);

else
    error("wrong type of input, dose_ligand must be string or numeric.")
end
end