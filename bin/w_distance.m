...
bw_max = max(bw_u_samples,bw_v_samples);
...
elseif p == 2
    ...
    bw_min = bw;      % initialize bandwidth
    bw_vec = linspace(bw_min, bw_max,10);

    for i_bw_vec = 1:length(bw_vec)
        % Clear any previous warning.
        lastwarn('');

        % Compute the kernel density estimate with the current bandwidth.
        [u_icdf,~] = ksdensity(u_samples,pts,...
            'Function','icdf','Bandwidth',bw);%,'Bandwidth',bw
        [v_icdf,~] = ksdensity(v_samples,pts,...
            'Function','icdf','Bandwidth',bw);%,'Bandwidth',bw

        % Check for any warnings that occurred during ksdensity.
        [warnMsg, ~] = lastwarn;

        % If no warning was produced, we are done.
        if isempty(warnMsg)
            break;
        elseif i_bw_vec == length(bw_vec)
            error('Does not converge for all bandwith, calculting w-dist')
        end

        % Otherwise, increase the bandwidth and try again.
        bw = bw_vec(i_bw_vec);
    end
    % 
    % [u_icdf,~] = ksdensity(u_samples,pts,...
