function [collect_feature_vects,metric_structs] = calculate_codon_from_metric202511_nonminmaxscaled(data_info,metric_structs) %, parameters)

% updated on 2025/11/17
% Using experiment-specific and simulation-specific dynamic metrics 
...
FeatureListFile_sim = parameters.CodewordListFile_sim;
FeatureListTable_sim = readtable(FeatureListFile_sim, 'Sheet', parameters.CodewordListFileSheet);
% FeatureList_sim = table2cell(FeatureListTable_sim(:,1));
FeatureSpecifiers_sim = table2cell(FeatureListTable_sim(:,2));
% CodewordList_sim = table2cell(FeatureListTable_sim(:,3));
    ...
    for j= 1:length(FeatureList)
        if strcmp(CodewordList{j}, codewords{i})
            for k=1:length(ids)
                ...
                if contains(lower(data_info.data_label{k} ), "sampling")
                    Feature_Specifiers_case = FeatureSpecifiers_sim{j};
                else
                    Feature_Specifiers_case = FeatureSpecifiers{j};
                end
                if abs(Feature_Specifiers_case) ~= 0
                    feature = feature(:, abs(Feature_Specifiers_case));
                end
                if Feature_Specifiers_case < 0
                    feature = feature*-1;
                end
...
parameters.CodewordListFile_sim=strcat('','FeatureList_v3_Dur3.xlsx');%./
